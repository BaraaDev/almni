...
@section('title') {{__('lecture.update lecture'). ' - ' . $model->name . $title}} @endsection
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{__('lecture.edit lecture') .': '. $model->title}}</h4>
                    </div>

                    <div class="card-body">
                        @include('layouts.admin.alert.validation-errors')
                        <div class="basic-form">
                            <form class="needs-validation" id="alert-form" method="post" action="{{route('lectures.update',$model->id)}}" files="true" enctype="multipart/form-data" novalidate>
                                @csrf
                                {{ method_field('put') }}
                                @include('admin.lectures.form')
                                <button type="submit" class="btn mt-3 me-2 btn-primary">{{__('home.update')}}</button>
                            </form>
                        </div>
                    </div>
                </div>
