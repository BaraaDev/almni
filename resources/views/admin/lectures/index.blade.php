...
@section('title') {{__('lecture.all lectures') . $title}} @endsection
@section('head')
                ...
                <div class="card students-list">
                    <div class="card-header border-0 flex-wrap pb-0">
                        ...
                        <div class="input-group search-area w-auto">
                            ...
                            <select class="form-select filter" aria-label="Default select example" id="filter-group">
                                <option value="0">{{__('group.groups')}}</option>
                                <option value="No Group">{{__('group.no group')}}</option>
                                @foreach($groups as $group)
                                    <option value="{{$group->name}}">{{$group->name}}</option>
                                @endforeach
                            </select>
                            <select class="form-select filter" aria-label="Default select example" id="filter-course">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <h4 class="mb-0 fs-16 font-w500">{{$lecture->name}}</h4>
                                            </div>
                                        </td>

                                        <td class="group" data-group="{{$lecture->group->name ?? __('group.no group')}}">{{$lecture->group->name ??  __('group.no group')}}</td>
                                        <td class="course" data-course="{{$lecture->course->title ?? __('course.no courses')}}">{{$lecture->course->title ?? __('course.no courses')}}</td>
                                        <td>{{$lecture->instructor->name ?? ''}}</td>
                                        <td>{{$lecture->created_at->translatedFormat('M d, Y')}}</td>
                                        <td class="sort" @if($lecture->status == 'active') data-sort="Active" @elseif($lecture->status == 'stopped') data-sort="Stopped" @endif >
                                            @if($lecture->status == 'active')
                                                <span class="badge  light badge-success">{{$lecture->status}}</span>
                                            @elseif($lecture->status == 'stopped')
                                                <span class="badge light badge-danger">{{$lecture->status}}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-start">
                                                    {!! Form::open([
                                                        'route' => ['lectures.destroy',$lecture->id],
                                                        'method' => 'delete'
                                                    ])!!}
                                                    @can('lecture-delete')
                                                    ...
                                                    @endcan
                                                    @can('lecture-edit')
                                                    <a class="dropdown-item" href="{{route('lectures.edit',$lecture->id)}}">{{__('home.edit')}}</a>
                                                    @endcan
                                                    <a class="dropdown-item" href="{{route('lectures.show',$lecture->id)}}">{{__('home.show')}}</a>
                                                    {!! Form::close() !!}
                                                </div>
                                            </div>
                                        </td>
        ...
        $(document).ready(function() {
            ...
            function filter_function(){
                ...
                $('#application-tbl1 tr').each(function() {
                    ...
                    else if(groupValue == $(this).find('td.group').data('group')){
                        groupFlag = 1;
                    }
                    else{
                        groupFlag = 0;
                    ...
                    else if(courseValue == $(this).find('td.course').data('course')){
                        courseFlag = 1;
                    }
                    else{
                        courseFlag = 0;
                    ...
                    if(groupFlag && sortFlag && courseFlag){
                        $(this).show();
                    }
                });
            }
