...
@section('title') {{$model->level . $title}} @endsection
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{__('level.edit level')}}</h4>
                    </div>

                    <div class="card-body">
                        @include('layouts.admin.alert.validation-errors')
                        <div class="basic-form">
                            <form class="needs-validation" id="alert-form" method="post" action="{{route('levels.update',$model->id)}}" files="true" enctype="multipart/form-data" novalidate>
                                @csrf
                                {{ method_field('put') }}
