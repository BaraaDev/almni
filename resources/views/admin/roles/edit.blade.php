...
@section('title') {{$role->name . $title}} @endsection
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{__('role.edit role')}}</h4>
                    </div>

                    <div class="card-body">
                        @include('layouts.admin.alert.validation-errors')
                        <div class="basic-form">
                            <form class="needs-validation" id="alert-form" method="post" action="{{route('roles.update',$role->id)}}" files="true" enctype="multipart/form-data" novalidate>
                                @csrf
                                {{ method_field('put') }}
                                <div class="row">
                                                        ...
                                                        @foreach($permission as $value)
                                                            <div class="col-md-6">
                                                                <div class="form-check custom-checkbox mb-3 checkbox-primary">
                                                                    <label class="form-check-label" for="customRadioBox{{$loop->iteration}}">
                                                                            {{ Form::checkbox('permission[]', $value->id, in_array($value->id, $rolePermissions) ? true : false, array('class' => 'name')) }}{{ $value->name }} </label>
